...
export default class Traces {
  ...
  constructor(
    ...
    material,
    name = "TRACE",
    layerName = PCB_LAYERS.TOP_COPPER,
  }) {
    ...
    // Add the trace to its logical PCB layer group (fallback to scene if no layerManager)
    if (this.layerManager?.add) {
      this.layerManager.add(this.layerName, this.mesh);
    } else {
